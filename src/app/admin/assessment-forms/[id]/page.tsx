...
import { getEligibleItemVersionsForPicker } from '@/services/scientificAdminService';
...
import { CloneFormButton } from '@/components/admin/forms/CloneFormButton';
...
export default async function AssessmentFormDetailPage({
  ...
  // Fetch eligible questions for question picker if form is in draft mode
  const eligibleQuestions = isDraft
    ? await getEligibleItemVersionsForPicker(form.id)
    : [];
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-200">
      ...
      <div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            >
              <Eye className="w-4 h-4 mr-1.5 text-brand-600" />
              Taslağı Önizle
            </Link>
            ...
            {!isDraft && (
              <span className="inline-flex items-center px-3 py-2 rounded-xl bg-surface-1 border border-border-subtle text-xs font-semibold text-text-secondary shadow-xs">
                <Lock className="w-3.5 h-3.5 mr-1.5 text-amber-600" />
                Dondurulmuş Form (Salt Okunur)
              </span>
            )}
          </div>
        </div>
      ...
      {isDraft ? (
        /* Editable Form Builder for DRAFT forms */
        <FormBuilder
          form={{
            id: form.id,
            versionCode: form.versionCode,
            ...
            items: form.items.map((fi) => ({
              ...
              itemVersionId: fi.itemVersionId,
              itemVersion: {
                ...
                status: fi.itemVersion.status,
                validationStatus: fi.itemVersion.validationStatus,
                authorType: fi.itemVersion.authorType,
                item: {
                  id: fi.itemVersion.item.id,
                  ...
                  itemType: fi.itemVersion.item.itemType,
                  facet: {
                    id: fi.itemVersion.item.facet.id,
                    nameTr: fi.itemVersion.item.facet.nameTr,
                    nameEn: fi.itemVersion.item.facet.nameEn,
                    construct: {
                      id: fi.itemVersion.item.facet.construct.id,
                      nameTr: fi.itemVersion.item.facet.construct.nameTr,
                      domain: {
                        id: fi.itemVersion.item.facet.construct.domain.id,
                        nameTr: fi.itemVersion.item.facet.construct.domain.nameTr,
                      },
                    },
                  },
                  instrument: fi.itemVersion.item.instrument
                    ? {
                        id: fi.itemVersion.item.instrument.id,
                        name: fi.itemVersion.item.instrument.name,
                        licenseType: fi.itemVersion.item.instrument.licenseType,
                        licensingDecision: fi.itemVersion.item.instrument.licensingDecision,
                      }
                    : null,
                },
              },
          ...
          eligibleQuestions={eligibleQuestions as any}
        />
      ) : (
        /* Frozen Read-Only View for PUBLISHED or ARCHIVED forms */
            ...
            <div className="divide-y divide-border-subtle">
              {form.items.map((formItem) => {
                ...
                const instDecision = normalizeInstrumentLicensingDecision(
                  itm.instrument?.licensingDecision
                );

                return (
                  <div
                    key={formItem.id}
                    className="p-5 hover:bg-bg-subtle/40 transition-colors space-y-3"
                  >
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                      ...
                      <div className="flex items-center space-x-2 text-xs">
                        <span className="text-text-tertiary font-medium">
                          {itm.facet.construct.domain.nameTr} &gt;{' '}
                          {itm.facet.construct.nameTr} &gt;{' '}
